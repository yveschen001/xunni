# XunNi 註冊引導流程設計

## 1. 概述

註冊流程採用**智能對話式引導**，支援**中斷後繼續**，並對關鍵資訊（性別、生日）進行**深度確認**。

---

## 2. 核心原則

### 2.1 不可修改的欄位

以下欄位**一旦設定，永遠不能修改**：
- **性別** (`gender`)
- **生日** (`birthday`) - 用於計算年齡和星座

### 2.2 年齡限制

- **未滿 18 歲**：不允許註冊，提示「請成年後再來」
- **年齡驗證**：必須輸入真實生日，系統計算年齡

### 2.3 中斷恢復機制

- 使用 `onboarding_state` 欄位記錄當前步驟
- 使用者中斷後，下次 `/start` 從中斷處繼續
- 支援查看已完成的步驟

---

## 3. 資料庫擴充

### 3.1 users 表新增欄位

```sql
-- 新增到 users 表
ALTER TABLE users ADD COLUMN birthday DATE;              -- 生日（用於計算年齡和星座）
ALTER TABLE users ADD COLUMN onboarding_state TEXT;     -- JSON: { step: 3, data: {...} }
ALTER TABLE users ADD COLUMN onboarding_started_at DATETIME;
ALTER TABLE users ADD COLUMN onboarding_completed_at DATETIME;
ALTER TABLE users ADD COLUMN terms_accepted INTEGER DEFAULT 0;  -- 是否接受條款
ALTER TABLE users ADD COLUMN privacy_accepted INTEGER DEFAULT 0; -- 是否接受隱私權
ALTER TABLE users ADD COLUMN terms_accepted_at DATETIME;
ALTER TABLE users ADD COLUMN privacy_accepted_at DATETIME;
```

### 3.2 onboarding_state JSON 結構

```typescript
interface OnboardingState {
  step: number;           // 當前步驟 (0-9)
  data: {
    nickname?: string;
    avatar_url?: string;
    language_pref?: string;
    gender?: string;
    birthday?: string;    // YYYY-MM-DD
    country?: string;
    prefer_gender?: string;
    mbti_answers?: number[];  // MBTI 測驗答案
    anti_scam_score?: number; // 反詐騙測驗分數
  };
  last_updated: string;   // ISO datetime
}
```

---

## 4. 完整註冊流程（10 步）

### Step 0：歡迎與條款同意

**智能對話**：
```
Bot: 👋 嗨！歡迎來到 XunNi～我是你的專屬引導小助手 ✨

讓我先簡單介紹一下：
XunNi 是一個匿名漂流瓶交友平台，透過 MBTI 和星座幫你找到志同道合的朋友！

在開始之前，我需要你同意我們的條款：
```

**顯示按鈕**：
- [📄 查看使用者條款]
- [🔒 查看隱私權政策]
- [✅ 我已閱讀並同意] (必須先查看才能同意)
- [❌ 不同意，離開]

**驗證邏輯**：
- 必須先點擊「查看使用者條款」和「查看隱私權政策」
- 兩個都查看後，「我已閱讀並同意」才可點擊
- 記錄 `terms_accepted` 和 `privacy_accepted` 為 1，記錄時間

**中斷恢復**：
- 如果已同意條款，跳過此步驟
- 如果未同意，從此步驟開始

---

### Step 1：暱稱 & 頭像

**智能對話**：
```
Bot: 太好了！讓我們開始設定你的個人資料吧 🎨

首先，給自己取一個有趣的暱稱吧～
（這是你給別人的第一印象，要好好想哦～）
```

**流程**：
1. 預設使用 Telegram `first_name` 或 `username`
2. AI 生成 1-3 個暱稱建議（根據語言、產品主題）
3. 使用者可：
   - 點擊建議暱稱
   - 輸入自訂暱稱
   - 使用預設值

**頭像設定**：
```
Bot: 接下來，設定你的頭像吧 📸

你可以：
1. 使用 Telegram 頭像
2. 上傳自訂頭像
3. 稍後再設定（可選）
```

**保存狀態**：
```typescript
onboarding_state = {
  step: 1,
  data: {
    nickname: '...',
    avatar_url: '...',
  },
  last_updated: '...'
}
```

---

### Step 2：主要使用語言

**智能對話**：
```
Bot: 🌍 你主要使用哪種語言呢？

這會影響：
- 介面顯示語言
- 聊天時的翻譯目標語言（VIP 功能）
```

**選項**：按鈕選擇（zh-TW, en, ja, ko, th, vi, ...）

---

### Step 3：性別（深度確認）

**智能對話**：
```
Bot: 👤 請告訴我你的性別

⚠️ 重要提醒：
性別設定後將**永遠不能修改**，請仔細選擇！
```

**AI 提示**（可選）：
```
Bot: 根據你的 Telegram 資料，我猜測你可能是 {ai_gender_hint}
（這只是建議，請根據實際情況選擇）
```

**選項**：
- [👨 男]
- [👩 女]
- [🌈 其他]

**深度確認**（選擇後）：
```
Bot: 你選擇了「{gender}」

⚠️ 再次確認：
性別設定後將**永遠不能修改**！

請確認：
[✅ 確認，這就是我的性別]
[❌ 重新選擇]
```

**最終確認**：
- 點擊「確認」後，寫入 `gender` 欄位
- 記錄確認時間
- 如果點擊「重新選擇」，回到選項

---

### Step 4：生日與年齡驗證

**智能對話**：
```
Bot: 🎂 請輸入你的生日（YYYY-MM-DD）

例如：2000-01-15

⚠️ 重要：
1. 生日設定後將**永遠不能修改**
2. 未滿 18 歲無法使用本服務
3. 請輸入真實生日，用於計算星座
```

**輸入驗證**：
1. 格式檢查：YYYY-MM-DD
2. 日期合理性檢查（不能是未來日期）
3. 年齡計算：`age = (today - birthday) / 365.25`
4. **18 歲驗證**：
   ```typescript
   if (age < 18) {
     return {
       error: 'AGE_RESTRICTION',
       message: '很抱歉，你未滿 18 歲，無法使用本服務。\n\n請成年後再來，我們等你！👋'
     };
   }
   ```

**深度確認**：
```
Bot: 你的生日是：{birthday}
計算出的年齡：{age} 歲
對應的星座：{zodiac_sign}

⚠️ 再次確認：
生日設定後將**永遠不能修改**！

請確認：
[✅ 確認，這是我的真實生日]
[❌ 重新輸入]
```

**保存**：
- 確認後寫入 `birthday` 和 `zodiac_sign`
- 計算 `age_range`（18-22 / 23-30 / 31-40 / 40+）

---

### Step 5：國家

**智能對話**：
```
Bot: 🌏 你來自哪個國家/地區？

這會幫助我們為你推薦更合適的配對～
```

**選項**：按鈕選擇（TW, JP, KR, TH, VN, US, ...）

---

### Step 6：喜好性向（目標對象）

**智能對話**：
```
Bot: 💕 你想認識什麼性別的朋友？

（這是你丟漂流瓶時的目標篩選條件）
```

**選項**：
- [👨 男]
- [👩 女]
- [🌈 其他]
- [🌍 不限]

---

### Step 7：MBTI 測驗

**智能對話**：
```
Bot: 🧠 讓我們來做個 MBTI 測驗吧！

這會幫助我們為你找到更合適的配對對象～
測驗共有 {totalQuestions} 題，每頁 3-5 題
```

**流程**：
1. 分頁顯示（每頁 3-5 題）
2. 每頁完成後：
   ```
   Bot: 太好了！你已經完成了 {current}/{total} 題
   繼續加油～ 💪
   ```
3. 完成後計算 MBTI 類型
4. 顯示結果：
   ```
   Bot: 🎉 測驗完成！

   你的 MBTI 類型是：{mbti_type}
   {mbti_description}
   ```

**中斷恢復**：
- 保存已答題目到 `onboarding_state.data.mbti_answers`
- 恢復時從未答題目繼續

---

### Step 8：反詐騙測驗

**智能對話**：
```
Bot: 🛡️ 最後一步！讓我們做個簡單的安全測驗

這會幫助我們建立一個更安全的環境～
共有 5 題情境題
```

**流程**：
1. 顯示情境題（每題 1 分）
2. 完成後計算分數
3. 得分 >= 3 分：通過
4. 得分 < 3 分：
   ```
   Bot: 很遺憾，你沒有通過測驗 😔

   別擔心！你可以重新測驗
   這次要更仔細閱讀題目哦～
   
   [🔄 重新測驗]
   ```

**中斷恢復**：
- 保存已答題目和分數
- 恢復時從未答題目繼續

---

### Step 9：完成註冊

**智能對話**：
```
Bot: 🎉 恭喜！你已經完成所有設定！

你的個人資料：
- 暱稱：{nickname}
- 性別：{gender}
- 年齡：{age} 歲
- 星座：{zodiac_sign}
- MBTI：{mbti_type}

現在你可以開始使用 XunNi 了！

[📦 丟個漂流瓶] [🔍 撿個瓶子] [👤 查看個人資料]
```

**保存**：
- `onboarding_completed_at` = now()
- `onboarding_state.step` = 9
- 標記使用者為「可用狀態」

---

## 5. 中斷恢復機制

### 5.1 恢復邏輯

```typescript
// src/domain/user.ts

export async function getOnboardingState(
  userId: string,
  db: D1Database
): Promise<OnboardingState | null> {
  const user = await db.getUser(userId);
  if (!user || !user.onboarding_state) {
    return null;
  }
  
  return JSON.parse(user.onboarding_state);
}

export async function resumeOnboarding(
  userId: string,
  db: D1Database
): Promise<number> {
  const state = await getOnboardingState(userId, db);
  
  if (!state) {
    return 0; // 從頭開始
  }
  
  // 檢查是否已完成
  if (state.step >= 9) {
    return -1; // 已完成
  }
  
  return state.step; // 從中斷處繼續
}
```

### 5.2 恢復提示

```
Bot: 👋 歡迎回來！

你上次的註冊進度：{step}/9

[▶️ 繼續註冊] [🔄 重新開始] [👀 查看已填寫資料]
```

---

## 6. 條款與隱私權

### 6.1 使用者條款

**內容**（需法律顧問審核）：
- 服務使用規範
- 使用者行為準則
- 違規處罰機制
- 免責聲明

**顯示方式**：
- 長文本，支援 Markdown 格式
- 可滾動查看
- 必須完整閱讀（檢測滾動到底部）才能同意

### 6.2 隱私權政策

**內容**（需法律顧問審核）：
- 資料收集範圍
- 資料使用目的
- 資料保護措施
- 使用者權利

**顯示方式**：同上

### 6.3 條款版本管理

```sql
CREATE TABLE terms_versions (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  type TEXT,              -- 'terms' / 'privacy'
  version TEXT,           -- '1.0', '1.1', ...
  content TEXT,           -- Markdown 格式
  effective_date DATE,
  created_at DATETIME
);

-- 使用者接受的版本記錄
ALTER TABLE users ADD COLUMN terms_version TEXT;
ALTER TABLE users ADD COLUMN privacy_version TEXT;
```

**版本更新**：
- 重大更新時，要求使用者重新同意
- 記錄使用者接受的版本號

---

## 7. 實作範例

### 7.1 處理 /start

```typescript
// src/telegram/handlers/start.ts

export async function handleStart(
  update: TelegramUpdate,
  env: Env,
  db: D1Database
): Promise<void> {
  const userId = String(update.message.from.id);
  
  // 1. 檢查使用者是否存在
  let user = await db.getUser(userId);
  
  if (!user) {
    // 新使用者：建立記錄
    user = await db.createUser({
      telegram_id: userId,
      onboarding_state: JSON.stringify({
        step: 0,
        data: {},
        last_updated: new Date().toISOString(),
      }),
      onboarding_started_at: new Date().toISOString(),
    });
  }
  
  // 2. 檢查是否已完成
  if (user.onboarding_completed_at) {
    await sendMessage(env, userId, '你已經完成註冊！使用 /help 查看可用指令');
    return;
  }
  
  // 3. 檢查中斷恢復
  const state = JSON.parse(user.onboarding_state || '{}');
  const currentStep = state.step || 0;
  
  // 4. 根據步驟處理
  switch (currentStep) {
    case 0:
      await handleTermsStep(userId, env, db);
      break;
    case 1:
      await handleNicknameStep(userId, env, db);
      break;
    // ... 其他步驟
  }
}
```

### 7.2 性別深度確認

```typescript
async function handleGenderConfirmation(
  userId: string,
  selectedGender: string,
  env: Env,
  db: D1Database
): Promise<void> {
  // 第一次選擇
  await sendMessage(env, userId, 
    `你選擇了「${selectedGender}」\n\n` +
    `⚠️ 再次確認：\n` +
    `性別設定後將**永遠不能修改**！\n\n` +
    `請確認：`,
    {
      reply_markup: {
        inline_keyboard: [
          [{ text: '✅ 確認，這就是我的性別', callback_data: `confirm_gender_${selectedGender}` }],
          [{ text: '❌ 重新選擇', callback_data: 'reselect_gender' }],
        ],
      },
    }
  );
}

// 處理確認回調
export async function handleGenderConfirmCallback(
  callbackQuery: CallbackQuery,
  env: Env,
  db: D1Database
): Promise<void> {
  const userId = String(callbackQuery.from.id);
  const data = callbackQuery.data;
  
  if (data.startsWith('confirm_gender_')) {
    const gender = data.replace('confirm_gender_', '');
    
    // 最終確認：寫入資料庫
    await db.prepare(`
      UPDATE users
      SET gender = ?,
          onboarding_state = json_set(
            onboarding_state,
            '$.step', 4,
            '$.data.gender', ?
          ),
          updated_at = datetime('now')
      WHERE telegram_id = ?
    `).bind(gender, gender, userId).run();
    
    await sendMessage(env, userId, '✅ 性別已確認！讓我們繼續～');
    await handleBirthdayStep(userId, env, db);
  }
}
```

---

## 8. 測試要點

1. **中斷恢復測試**：註冊到一半中斷，重新進入應從中斷處繼續
2. **年齡驗證測試**：未滿 18 歲應被拒絕
3. **性別確認測試**：必須經過兩次確認才能設定
4. **生日確認測試**：必須經過兩次確認才能設定
5. **條款同意測試**：必須先查看才能同意

---

## 9. 注意事項

1. **法律合規**：條款和隱私權政策需法律顧問審核
2. **資料保護**：生日等敏感資訊需加密存儲（可選）
3. **使用者體驗**：智能對話要友好、不冗長
4. **錯誤處理**：妥善處理使用者輸入錯誤、網路中斷等情況

