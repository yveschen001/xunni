...
> **⚠️ 重要：在開始任何編輯前，必須先閱讀 `@doc/SPEC.md`（專案規格書，包含專案概覽和完整規格）**

本目錄包含 XunNi 專案的完整開發規範與指南。請按照以下順序閱讀：

1. **首先閱讀**: `@doc/SPEC.md` - 專案規格書（包含專案概覽、結構和完整業務邏輯）
2. **設置環境**: `@doc/ENV_CONFIG.md` - 開發環境設置指南（本地環境設置）
...
1. 閱讀 **`@doc/SPEC.md`** 了解專案概覽、結構和完整業務邏輯（**必讀**）
   - 特別是附錄「術語表 / Glossary」（了解核心詞彙定義）
2. 閱讀 **`@doc/ENV_CONFIG.md`** 設置本地開發環境
   - 特別是第 6 節「開發前檢查清單」（確保所有準備事項已完成）
3. 閱讀 **`@doc/DEVELOPMENT_STANDARDS.md`** 了解開發規範
   - 特別是第 5 節「與 AI 協作流程」（了解如何使用 AI 進行開發）
4. 閱讀 **`@doc/MODULE_DESIGN.md`** 了解架構設計
