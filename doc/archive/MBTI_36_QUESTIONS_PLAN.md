...
開發完整的 MBTI 36 題測試，提供更準確的性格測試結果。

**重要：** 保留現有的 12 題快速版本（用於註冊流程），新增 36 題完整版本（用於重新測試）。
...
**兩個版本：**
1. **12 題快速版本** - 註冊流程使用（已完成，保留）
2. **36 題完整版本** - 重新測試使用（待開發）

**測試入口：**
1. **註冊流程** - 使用 12 題快速版本（保持不變）
2. **重新測試** - 提供版本選擇：
   - `/mbti` 命令
...
export interface MBTITestSession {
  version: MBTITestVersion;  // 'quick' or 'full'
  currentQuestion: number;  // 1-12 or 1-36
  totalQuestions: number;   // 12 or 36
...
// 獲取指定版本的題目
export function getMBTIQuestions(version: MBTITestVersion): MBTIQuestion[] {
  return version === 'quick' ? MBTI_QUESTIONS_QUICK : MBTI_QUESTIONS_FULL;
}
