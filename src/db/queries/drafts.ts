 ...
 * Delete draft
 */
export async function deleteDraft(db: DatabaseClient, draftId: number): Promise<void> {
  await db.d1
    .prepare(
      `
    DELETE FROM bottle_drafts
 ...
 */
export async function deleteUserDrafts(db: DatabaseClient, telegramId: string): Promise<void> {
  await db.d1
    .prepare(
      `
    DELETE FROM bottle_drafts
 ...
 */
export async function cleanupExpiredDrafts(db: DatabaseClient): Promise<number> {
  const result = await db.d1
    .prepare(
      `
    DELETE FROM bottle_drafts
