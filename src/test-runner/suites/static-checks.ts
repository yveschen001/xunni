...
export class StaticChecksSuite extends TestSuite {
  ...
  async run(context: TestContext): Promise<TestResult[]> {
    ...
    results.push(await this.runTest('Check Hardcoded Chinese', async () => {
      this.checkHardcodedChinese();
    }));

    // 2. Check i18n Key Completeness (Base vs All Locales)
    results.push(await this.runTest('i18n Key Completeness', async () => {
      this.checkI18nCompleteness();
    }));

    // 3. Check Database Migrations (Local only for safety)
    if (context.env === 'local') {
      results.push(await this.runTest('Database Migrations Check', async () => {
  ...
  private checkI18nCompleteness() {
    ...
    const files = fs.readdirSync(localesDir).filter(f => f.endsWith('.ts') && f !== 'zh-TW.ts' && f !== 'types.ts' && f !== 'template.ts');
