...
export class MoonPacketSuite extends TestSuite {
  ...
  async run(context: TestContext): Promise<TestResult[]> {
    ...
    results.push(
      await this.runTest('Mode A: Get Rules', async () => {
        ...
        if (!json.data || Array.isArray(json.data))
          throw new TestFailure('Invalid response structure (data should be object)');
    ...
    results.push(
      await this.runTest('Mode B: Get User Profile', async () => {
        const headers = this.generateHeaders({}, TEST_KEY, TEST_SECRET);
        ...
        if (missing.length > 0)
          throw new TestFailure(`Missing fields: ${missing.join(', ')}`, json.data);
      })
    );

    // 3. Test Invalid Signature (Security)
    results.push(
      await this.runTest('Security: Reject Invalid Signature', async () => {
