...
export class TestRunner {
  ...
  async runAll(): Promise<boolean> {
    ...
    console.log(`🚀 Starting Smoke Tests with concurrency ${this.concurrency} on ${this.context.env}...`);
    ...
    for (let i = 0; i < this.suites.length; i += this.concurrency) {
      ...
      const chunkPromises = chunk.map(async (suite) => {
        ...
        try {
          ...
          console.log(`${symbol} [${suite.name}] Finished (${results.length} tests, ${failed.length} failed)`);
          
          // Print failures immediately for debugging
          failed.forEach(f => {
            console.error(`   🛑 ${f.name}: ${f.error}`);
            if (f.context) console.error('      Context:', JSON.stringify(f.context).slice(0, 200) + '...');
          });
