...
import { CompatibilityEngine, type MatchTopic, type MatchRecommendation } from '~/domain/compatibility/engine';
 ...
 */
export async function handleMatchPush(env: Env, db: D1Database, targetUserId?: string): Promise<void> {
  ...
  console.log(`[MatchPush] Starting weekly match push. Topic: ${currentTopic} (Week ${weekNumber})`);
  if (targetUserId) {
  ...
  if (targetUserId) {
    ...
    if (isQuietHours(now, preferences)) {
      console.log(`[MatchPush] ⚠️ TEST MODE: User ${user.telegram_id} is in QUIET HOURS but proceeding anyway.`);
    ...
    if (!recommendation.isValid) {
      console.log(`[MatchPush] Target user ${targetUserId} has no valid recommendation for ${currentTopic}.`);
      return;
    }
...
async function sendMatchNotification(
  ...
  const targetDisplay = recommendation.topic === 'zodiac' 
