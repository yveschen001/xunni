...
export async function handleBroadcastFilter(message: any, env: Env) {
  ...
  const messageBody = firstSpaceIndex === -1 ? '(Dry Run No Content)' : text.substring(firstSpaceIndex + 1).trim();
  ...
  const result = await db.d1.prepare(`
    ...
    RETURNING id
  `).bind(messageBody, JSON.stringify(filters), userId, count).first<{ id: number }>();
