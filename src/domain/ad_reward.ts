 ...
 * }
 */
export function canWatchAd(adReward: AdReward | null, isVIP: boolean): AdRewardCheckResult {
  // VIP users cannot watch ads (they don't need quota)
  if (isVIP) {
    return {
      can_watch: false,
      remaining_ads: 0,
      reason: 'VIP users have unlimited quota and cannot watch ads',
    };
  }
  ...
  if (remainingAds <= 0) {
    return {
      can_watch: false,
      remaining_ads: 0,
      reason: 'Daily ad limit reached (20/20)',
    };
  }
...
export function processAdCompletion(
  ...
  const checkResult = canWatchAd(adReward, isVIP);
  ...
  if (!checkResult.can_watch) {
    return {
      ...
      remaining_ads: 0,
      message: checkResult.reason || 'Cannot watch more ads',
    };
  }
  ...
  return {
    ...
    message: `Ad completed! Earned +${AD_REWARD_CONSTANTS.QUOTA_PER_AD} quota`,
  };
}
 ...
 * // "📺 今日廣告：5/20 | 已獲得 5 個額度"
 */
export function formatAdRewardStatus(adReward: AdReward | null, isVIP: boolean): string {
  if (isVIP) {
    return '💎 VIP 用戶無需觀看廣告';
  }

  if (!adReward) {
    return `📺 今日廣告：0/${AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} | 已獲得 0 個額度`;
  }
  ...
  if (remaining === 0) {
    return `📺 今日廣告：${adsWatched}/${AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} ✅ 已達上限 | 已獲得 ${quotaEarned} 個額度`;
  }

  return `📺 今日廣告：${adsWatched}/${AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} | 已獲得 ${quotaEarned} 個額度 | 剩餘 ${remaining} 次`;
}
