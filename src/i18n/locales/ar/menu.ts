export default {
  "bottle": "• يتم تفعيلها بعد أن يرمي الأصدقاء زجاجة الرسائل الأولى\n",
  "buttonCatch": "🎣 التقط زجاجة البركة من البركة",
  "buttonChats": "💬 محادثاتي",
  "buttonHelp": "❓ مساعده",
  "buttonInvite": "👥 دعوة الأصدقاء",
  "buttonProfile": "👤 الملف الشخصي",
  "buttonSettings": "⚙️ الإعدادات",
  "buttonStats": "📊 الإحصائيات",
  "buttonThrow": "🌊 ألقِ زجاجة البركة من البركات",
  "buttonVip": "💎 العضوية المميزة",
  "invite": "🎁 **دعوة الأصدقاء**",
  "invite2": "📋 رمز الدعوة الخاص بك: {inviteCode}",
  "invite3": "📤 شارك رمز الدعوة",
  "levelFree": "🆓 عضو مجاني",
  "levelVip": "💎 عضو مميز",
  "message": "XunNi ألقِ زجاجة البركة معًا! 🍾 استخدم رمز الدعوة الخاص بي للانضمام، وسنحصل جميعًا على المزيد من البركات!",
  "message2": "XunNi ألقِ زجاجة البركة معًا! 🍾 استخدم رمز الدعوة الخاص بي للانضمام، وسنحصل جميعًا على المزيد من البركات!",
  "notRegistered": "غير مسجل",
  "notSet": "غير محدد",
  "quota": "• تحصل كلاكما على حصة يومية +1\n\n",
  "register": "• الأصدقاء يستخدمون رمز الدعوة الخاص بك للتسجيل\n",
  "selectFeature": "يرجى اختيار ميزة:",
  "settings": "• MBTI: {mbti}",
  "settings2": "• برج: {zodiac}\n\n",
  "settings3": "غير محدد",
  "settings4": "غير محدد",
  "short": "عضو مجاني",
  "stats": "📊 عرض إحصائيات الدعوات: /profile",
  "stats2": "📊 عرض إحصائيات الدعوات",
  "task": "🎯 **المهمة التالية**\n⏳ {nextTask.name} (+{nextTask.reward_amount} زجاجة رسائل)\n💡 {nextTask.description}",
  "text": "🏠 **القائمة الرئيسية** {vipBadge}\n\n",
  "text2": "👋 مرحبًا، {user.nickname}!\n\n",
  "text3": "💡 انقر على الزر أدناه لمشاركة مع الأصدقاء: \n",
  "text4": "💡 اختر الميزة التي تريدها:",
  "text5": "📊 حالتك: \n",
  "title": "🏠 **القائمة الرئيسية**",
  "userNotFound": "المستخدم غير موجود",
  "vip": "• المستوى: {isVip ? 'عضو مميز 💎' : 'عضو مجاني'} {isVip ? 'عضو مميز 💎' : 'عضو مجاني'} {isVip ? 'VIP Member 💎' : 'Free Member'} {isVip ? 'VIP 會員 💎' : '免費會員'}",
  "vip2": "عضو VIP 💎",
  "yourStatus": "حالتك",
  "birthDate": "عيد الميلاد",
  "zodiac": "برجك الفلكي",
  "bloodType": "فصيلة الدم",
  "interests": "الاهتمامات",
  "bio": "الملف الشخصي",
  "driftBottles": "زجاجة البركة",
  "fortuneBottles": "زجاجة الحظ"
};
