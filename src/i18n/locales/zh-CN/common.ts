export default {
  "ad": "📺 今日广告：{adsWatched}/{AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} | 已获得 {quotaEarned} 个额度 | 剩余 {remaining} 次",
  "ad2": "📺 今日广告：{adsWatched}/{AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} ✅ 已达上限 | 已获得 {quotaEarned} 个额度",
  "ad3": "📺 今日广告：0/{AD_REWARD_CONSTANTS.MAX_ADS_PER_DAY} | 已获得 0 个额度",
  "ad4": "• 📺 观看广告（剩余 {remaining}/20 次）\n",
  "ad5": "• 📺 观看广告（今日已达上限）\n",
  "ad6": "• 避免广告或不当内容\n\n",
  "ad7": "📊 暂无官方广告",
  "ad8": "📢 垃圾广告",
  "ad9": "💡 还有更多官方广告可以观看！",
  "admin": "请稍后再试，或联系管理员。",
  "age": "无效的年龄范围：{trimmedValue}（格式必须是 min-max，如 18-25）",
  "age2": "年龄：{filters.age.min}-{filters.age.max} 岁",
  "age3": "无效的年龄范围：{trimmedValue}（年龄必须在 18-99 之间）",
  "age4": "无效的年龄范围：{trimmedValue}（最小年龄不能大于最大年龄）",
  "anonymous": "匿名",
  "anonymousUser": "匿名用户",
  "anyBloodType": "🌈 任何血型",
  "anyone": "🌈 任何人",
  "back": "↩️ 返回",
  "back2": "↩️ 返回编辑资料",
  "back3": "🏠 返回主选单",
  "back4": "↩️ 返回",
  "backToMenu": "🏠 返回主菜单",
  "backToMainMenu": "返回主菜单",
  "birthday": "🎂 生日: {updatedUser.birthday}",
  "birthday2": "🎂 生日: {user.birthday}",
  "birthday3": "当天生日",
  "bloodType": "🩸 血型: {bloodTypeText}",
  "bloodType2": "🩸 **编辑血型**\n\n",
  "bloodType3": "请选择你的血型：",
  "bloodType4": "🩸 编辑血型",
  "bloodTypeA": "🩸 A型",
  "bloodTypeAB": "🩸 AB型",
  "bloodTypeB": "🩸 B型",
  "bloodTypeO": "🩸 O型",
  "bottle": "瓶子内容太短，至少需要 {MIN_BOTTLE_LENGTH} 个字符（目前 {trimmedContent.length} 个字符）",
  "bottle10": "奖励：+1 瓶子（已追加到今天的额度）\n\n",
  "bottle11": "你们将不会再被匹配到对方的祝福漂流瓶。",
  "bottle12": "你想要送祝福漂流瓶时配对什么样的对象？",
  "bottle13": "使用 /throw 发送祝福漂流瓶开始第一步幸福的聊天吧！",
  "bottle14": "瓶子内容包含不适当的内容，请修改后重新提交",
  "bottle15": "点击下方按钮领取奖励：+1 瓶子\n\n",
  "bottle16": "💡 下次丢祝福漂流瓶时将自动使用此设置。",
  "bottle17": "🌊 发送祝福漂流瓶 - /throw",
  "bottle18": "🎣 捡起祝福漂流瓶 - /catch",
  "bottle19": "🎉 确认后可获得 +1 瓶子奖励！",
  "bottle2": "瓶子内容太长，最多 {MAX_BOTTLE_LENGTH} 个字符（目前 {content.length} 个字符）",
  "bottle20": "✏️ 请输入新的祝福漂流瓶内容：",
  "bottle21": "• 使用 /catch 捡新的祝福漂流瓶",
  "bottle22": "• /throw - 丢出祝福漂流瓶",
  "bottle23": "• /catch - 捡起祝福漂流瓶",
  "bottle24": "• /throw - 丢祝福漂流瓶",
  "bottle25": "• /catch - 捡祝福漂流瓶",
  "bottle26": "• 发送草稿内容来丢出祝福漂流瓶",
  "bottle27": "📦 **丢出祝福漂流瓶**",
  "bottle28": "🎣 **捡起祝福漂流瓶**",
  "bottle29": "💡 完成任务可获得额外祝福漂流瓶",
  "bottle3": "• 祝福漂流瓶: {bottlesCount?.count }",
  "bottle30": "瓶子内容不允许包含任何连结",
  "bottle31": "🍾 丢祝福漂流瓶",
  "bottle32": "瓶子内容不能为空",
  "bottle33": "丢出第一个瓶子",
  "bottle34": "捡起第一个瓶子",
  "bottle4": "匿名祝福漂流瓶交友平台，通过彼此祝福，以及 MBTI 和星座帮助你找到志同道合属性合适的朋友",
  "bottle5": "⏰ 对话已超时\n\n对方可能已离开。使用 /catch 捡新的瓶子吧！",
  "bottle6": "💡 使用 /catch 捡新的祝福漂流瓶开始新对话。",
  "bottle7": "🍾 丢祝福漂流瓶\n\n你想要寻找什么样的聊天对象？先给远方的他/她送上祝福吧~幸福会回到你身上的~",
  "bottle8": "快去丢祝福漂流瓶，和新朋友彼此祝福吧！/throw",
  "bottle9": "看看别人的祝福漂流瓶，彼此祝福~回复开始聊天",
  "broadcast": "广播将在后台发送，使用 /broadcast_status {broadcastId} 查看进度。",
  "broadcast10": "📊 最近 5 条广播记录\n\n",
  "broadcast11": "维护通知已广播给所有用户。\n",
  "broadcast12": "恢复通知已广播给所有用户。",
  "broadcast13": "📊 目前没有广播记录",
  "broadcast14": "📊 广播状态\n\n",
  "broadcast15": "所有广播状态正常。",
  "broadcast2": "大规模广播需要升级系统架构，请参考 BROADCAST_SYSTEM_REDESIGN.md",
  "broadcast3": "使用 /broadcast_status 查看所有广播记录。",
  "broadcast4": "{statusEmoji} 广播队列处理已触发\n\n",
  "broadcast5": "/broadcast_cancel\n\n",
  "broadcast6": "广播 ID: {ids.join(', ')}\n\n",
  "broadcast7": "正在处理广播 #{broadcast.id}\n",
  "broadcast8": "目前没有待处理或卡住的广播。\n\n",
  "broadcast9": "广播 ID: {ids.join(",
  "cancel": "请移除这些连结后重新输入或取消编辑：",
  "cancel2": "状态: 已取消\n\n",
  "cancel3": "请重新输入或取消编辑：",
  "cancelled": "已取消",
  "catch": "⏰ 捡瓶流程已超时\n\n请使用 /catch 重新开始。",
  "catch2": "你捡瓶回覆 → 对方也回覆 → 开始匿名聊天",
  "catch3": "• 友善、尊重的内容更容易被捡到哦～",
  "catch4": "捡瓶流程",
  "close": "❌ 关闭",
  "complete": "🎉 **广告观看完成！**\n\n✅ 获得 **+1 个额度**\n📊 今日已观看：**{updated.ads_watched}/20** 次\n🎁 今日已获得：**{updated.quota_earned}** 个额度\n📈 剩余次数：**{result.remaining_ads}** 次\n\n{result.remaining_ads > 0 ? '💡 继续观看广告可获得更多额度！' : '✅ 今日广告已达上限'}",
  "complete2": "📺 **观看广告获得额度**\n\n🎁 完成观看可获得 **+1 个额度**\n📊 今日剩余：**{remainingAds}/20** 次\n\n👇 点击下方按钮开始观看",
  "complete3": "完成时间: {new Date(broadcast.completedAt).toLocaleString('zh-TW')}\n",
  "complete4": "预计完成：{new Date(maintenance.endTime).toLocaleString('zh-TW')}\n",
  "complete5": "🎉 {testTitle} 完成了！",
  "complete6": "在截止时间前完成",
  "complete7": "即将完成",
  "complete8": "尽早完成",
  "confirm": "确认",
  "confirm2": "🌍 **确认你的国家/地区**\n\n",
  "confirm3": "🛡️ 最后一步：反诈骗安全确认\n\n",
  "confirm4": "🔍 **广播过滤器确认**\n\n",
  "confirm5": "🌍 确认你的国家/地区",
  "confirm6": "**确认清理？ **\n",
  "confirm7": "请确认：",
  "conversation": "📨 {formatIdentifier(conv.identifier)} 的对话（{conv.message_count} 则讯息）\n",
  "conversation10": "💡 **没有找到对话历史**\n\n",
  "conversation11": "部分对话历史可能未能更新，请稍后再试。",
  "conversation12": "📨 **最近对话：**\n\n",
  "conversation13": "💬 你还没有任何对话记录\n\n",
  "conversation14": "💬 继续对话：/reply\n",
  "conversation15": "您还没有任何对话记录。\n\n",
  "conversation16": "对话可能已结束或不存在。",
  "conversation17": "💬 继续对话",
  "conversation18": "开始第一次对话",
  "conversation2": "💬 **与 {formatIdentifier(identifier)} 的对话**\n\n",
  "conversation3": "• 对话开始：{formatDate(stats.first_message_time)}\n",
  "conversation4": "• 对话: {conversationsCount?.count }\n",
  "conversation5": "💬 回覆对话 {conversationIdentifier}",
  "conversation6": "您的头像缓存已刷新，下次查看对话历史时将显示最新头像。\n\n",
  "conversation7": "💡 为了保护隐私和安全，对话中只允许纯文字讯息。\n\n",
  "conversation8": "使用 /history 查看所有对话\n\n",
  "conversation9": "🔄 正在刷新所有对话历史...\n\n",
  "country": "无效的国家/地区代码：{trimmedValue}（必须是 2 个大写字母，如 US, JP）",
  "country2": "🌍 **请选择你的国家/地区**",
  "country3": "国家/地区：{filters.country}",
  "end": "结束：{endTime.toLocaleString('zh-TW')}\n\n",
  "end2": "结束：{endTime.toLocaleString(",
  "female": "女",
  "free": "免费",
  "gender": "👤 性别：{gender}\n",
  "gender2": "无效的性别值：{trimmedValue}（必须是 male, female 或 other）",
  "gender3": "👤 性别：{gender}\n",
  "gender4": "👤 性别：{updatedUser.gender ===",
  "gender5": "👤 性别：{user.gender ===",
  "gender6": "现在请选择你的性别：\n\n",
  "gender7": "其他性别",
  "help": "MBTI 性格测验可以帮助我们为你找到更合适的聊天对象～\n\n",
  "help2": "❓ 查看帮助 - /help",
  "help3": "• /help - 查看帮助",
  "invite": "• 邀请记录总数: {inviteStats?.total }\n",
  "invite2": "邀请码: {user.invite_code }\n",
  "invite3": "被谁邀请: {user.invited_by }\n\n",
  "loading": "✅ 正在加载......",
  "login": "一般用户将无法使用服务，只有管理员可以登入。",
  "male": "男",
  "mbti": "无效的 MBTI 类型：{trimmedValue}（必须是以下之一：{VALID_MBTI.join(', ')}）",
  "mbti10": "✍️ 手动输入 MBTI",
  "mbti11": "🧠 MBTI 选单",
  "mbti12": "MBTI 完整测验",
  "mbti13": "MBTI 快速测验",
  "mbti2": "你的 MBTI 类型是：**{result.type}**\n\n",
  "mbti3": "当前 MBTI：**{mbti}**\n",
  "mbti4": "🧠 **选择 MBTI 测验版本**\n\n",
  "mbti5": "🧠 **MBTI 性格类型管理**\n\n",
  "mbti6": "⚙️ 进阶筛选（MBTI/星座）",
  "mbti7": "• 手动修改你的 MBTI 类型",
  "mbti8": "请选择你的 MBTI 类型：",
  "mbti9": "🧠 重新测试 MBTI",
  "message": "3. 遇到诈骗请立即举报",
  "message10": "/broadcast_filter gender=female,age=18-25,country=TW 大家好！",
  "message11": "{banHours} {user.language_pref === 'en' ? 'hours' : '小时'}",
  "message12": "时间: {new Date(b.created_at).toLocaleString('zh-CN')}",
  "message13": "队列中还有 {pendingBroadcasts.results.length - 1} 个广播待处理",
  "message14": "进度: {broadcast.sent_count}/{broadcast.total_users}",
  "message15": "{days} {user.language_pref === 'en' ? 'days' : '天'}",
  "message16": "目标: {getBroadcastTargetName(broadcast.targetType)}",
  "message17": "状态：{maintenance.isActive ? '✅ 维护中' : '❌ 未启用'}",
  "message18": "🚫 禁止的网址：\n{urlCheck.blockedUrls?.map((url) =>",
  "message19": "• 最后消息：{formatDate(stats.last_message_time)}",
  "message2": "4. 官方不会私下索取资料",
  "message20": "时间：{formatDate(conv.last_message_time)}",
  "message21": "预计时长：{maintenance.estimatedDuration} 分钟",
  "message22": "使用 /broadcast_status {broadcastId} 查看进度。",
  "message23": "💡 请长按你要回复的消息，在出现的菜单中选择「回复」后，在聊天框中输入回复内容。",
  "message24": "📊 今日已发送：{usedToday + 1}/{dailyLimit} 则",
  "message25": "/broadcast_filter zodiac=Scorpio 天蝎座专属消息",
  "message26": "• 已激活: {inviteStats?.activated }",
  "message27": "• 待激活: {inviteStats?.pending }",
  "message28": "/maintenance_enable <时长(分钟)> [维护消息]",
  "message29": "进度: {b.sent_count}/{b.total_users}",
  "message3": "进度: {broadcast.sentCount}/{broadcast.totalUsers} ({progress.percentage}%)",
  "message30": "• 消息: {messagesCount?.count }",
  "message31": "• 对方发送：{stats.partner_messages} 则\n",
  "message32": "指挥官 - 大胆、富有想像力且意志强大的领导者，总能找到或创造解决方法。",
  "message33": "💡 使用 /broadcast_status <id> 查看详细信息",
  "message34": "🏷️ 兴趣标签：{updatedUser.interests ||",
  "message35": "执政官 - 极有同情心、受欢迎且乐于助人的人，总是渴望为社群做出贡献。",
  "message36": "/broadcast_filter <filter> <message content>",
  "message37": "**消息内容：**\n{broadcastMessage}",
  "message38": "• 总消息数：{stats.total_messages} 条",
  "message39": "竞选者 - 热情、有创造力且社交能力强的自由精神，总能找到理由微笑。",
  "message4": "💡 使用 /history {formatIdentifier(conversations[0].identifier)} 查看完整对话",
  "message40": "剩余时间：{remaining.remainingText}\n",
  "message41": "表演者 - 自发、精力充沛且热情的表演者，生活在他们周围从不无聊。",
  "message42": "用户数：{broadcast.total_users} 人",
  "message43": "• 你发送：{stats.user_messages} 则",
  "message44": "调停者 - 诗意、善良的利他主义者，总是热情地为正义事业而努力。",
  "message45": "请稍后使用 /broadcast_status 查看进度。",
  "message46": "昵称: {user.nickname }",
  "message47": "• 奖励：{stats.total_rewards}",
  "message48": "企业家 - 聪明、精力充沛且善于洞察的人，真正享受生活在边缘。",
  "message49": "🎁 奖励：+{ad.reward_quota} 个永久额度",
  "message5": "时间：{new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' })}\n",
  "message50": "时间：{new Date().toLocaleString(",
  "message51": "使用 /broadcast_status 查看更新后的状态。",
  "message52": "目标: {broadcast.target_type}\n",
  "message53": "使用 /broadcast_status 查看更新后的记录。",
  "message54": "• 如果您更换了 Telegram 头像，系统会自动检测\n",
  "message55": "约 {Math.ceil(totalSeconds)} 秒",
  "message56": "启用者：{maintenance.enabledBy}\n",
  "message57": "逻辑学家 - 具有创新精神的发明家，对知识有着止不住的渴望。",
  "message58": "提倡者 - 安静而神秘，同时鼓舞人心且不知疲倦的理想主义者。",
  "message59": "守卫者 - 非常专注且温暖的守护者，时刻准备着保护所爱之人。",
  "message6": "• 展示：{stats.total_views} | 点击：{stats.total_clicks} ({stats.ctr}%)\n",
  "message60": "探险家 - 灵活且迷人的艺术家，时刻准备着探索和体验新事物。",
  "message61": "• 长按对方讯息回覆 /block 可封锁此使用者\n",
  "message62": "请确保回覆的是对方发送的讯息（带有 # 标识符）。",
  "message63": "/broadcast_non_vip <消息内容>",
  "message64": "建立你的第一个连接（长按讯息 → 选择「回覆」）",
  "message65": "讯息: {messagePreview}\n",
  "message66": "3. 遇到可疑讯息时，你会提高警觉吗？\n\n",
  "message67": "💡 VIP 用户每日可发送 100 则讯息。",
  "message68": "/broadcast <msg_content>",
  "message69": "/broadcast_vip <msg_content>",
  "message7": "• 验证次数: {stats.total_verified}\n• 验证率: {stats.verification_rate}%",
  "message70": "最后讯息：{preview}\n",
  "message71": "广播讯息不能超过 4000 个字符",
  "message72": "1️⃣ 长按对方的讯息\n",
  "message73": "请使用文字讯息与对方交流。",
  "message74": "(尚无讯息)\n\n",
  "message75": "获取最新消息和活动",
  "message76": "广播讯息不能为空",
  "message77": "(无讯息)",
  "message8": "• 奖励发放: {stats.total_rewards}\n• 奖励率: {stats.reward_rate}%",
  "message9": "• 剩余展示: {ad.max_views - ad.current_views}/{ad.max_views}",
  "newUser": "新用户",
  "next": "下一页",
  "nickname": "💡 请输入一个简单的昵称，不要包含 http:// 或 https:// 等连结。\n\n",
  "nickname10": "请输入新的昵称：\n\n",
  "nickname11": "• 请勿使用昵称发送广告",
  "nickname12": "📝 编辑昵称",
  "nickname13": "✍️ 自订昵称",
  "nickname2": "很好！你的昵称是：{truncatedNickname}\n\n",
  "nickname3": "📝 昵称：{updatedUser.nickname}\n",
  "nickname4": "📝 昵称：{user.nickname}\n",
  "nickname5": "• 昵称长度限制 36 个字\n",
  "nickname6": "📝 **编辑昵称**\n\n",
  "nickname7": "✏️ 请选择你的昵称：\n\n",
  "nickname8": "✏️ 请输入你的昵称：\n\n",
  "nickname9": "请告诉我你的昵称（显示名称）：",
  "no": "否",
  "none": "无",
  "notRegistered": "未注册",
  "notSet": "未设定",
  "open": "打开",
  "operationFailed": "❌ 发生错误",
  "pageInfo": "第 {current} / {total} 页",
  "prev": "上一页",
  "profile": "👤 查看个人资料 - /profile\n",
  "profile2": "✏️ **编辑个人资料**\n\n",
  "profile3": "（你也可以稍后在个人资料中设置）",
  "quota": "💡 升级 VIP 可获得更多配额（100 则/天）：/vip",
  "quota2": "• 💎 升级 VIP（每天 30 个配额）",
  "quota3": "• 🎁 邀请好友（每人 +1 配额）\n",
  "quota4": "• ✨ 完成任务（获得永久配额）\n",
  "register": "💡 这是快速测验（{testInfo}），结果仅供参考。\n完成注册后，可使用 /mbti 重新测验。",
  "register10": "🎉 恭喜完成注册！\n\n",
  "register2": "💡 这是完整测验（{testInfo}），结果更准确。\n完成注册后，可使用 /mbti 重新测验。",
  "register3": "注册步骤: {user.onboarding_step}\n",
  "register4": "⏰ 注册流程已超时\n\n请使用 /start 重新开始注册。",
  "register5": "或使用：/dev_restart（自动开始注册）\n\n",
  "register6": "💡 现在可以重新开始测试注册流程。\n\n",
  "register7": "🔄 重新注册：/start\n",
  "register8": "💡 完成注册后，你可以：\n",
  "register9": "已自动完成注册流程。\n\n",
  "report": "🚨 **举报不当内容** (#{conversationIdentifier})\n\n",
  "report2": "多次被举报 / Multiple reports",
  "report3": "💡 这样可以准确指定要举报的对象。",
  "report4": "请选择举报原因：",
  "selected": "已选择",
  "settings": "🧠 MBTI：{updatedUser.mbti_result }（可重新测试）",
  "settings10": "你还没有设定 MBTI 类型。\n\n",
  "settings11": "设定地区",
  "settings12": "未设定",
  "settings13": "未设定",
  "settings14": "未设定",
  "settings15": "未设定",
  "settings16": "未设定",
  "settings17": "未设定",
  "settings18": "未设定",
  "settings19": "未设定",
  "settings2": "🏷️ 兴趣标签：{updatedUser.interests}\n",
  "settings20": "未设定",
  "settings21": "未设定",
  "settings22": "未设定",
  "settings23": "未设定",
  "settings24": "未设定",
  "settings25": "未设定",
  "settings26": "未设定",
  "settings27": "未设定",
  "settings28": "未设定",
  "settings29": "未设定",
  "settings3": "🧠 MBTI：\\$（可重新测试）",
  "settings30": "未设定",
  "settings31": "未设定",
  "settings32": "未设定",
  "settings33": "未设定",
  "settings34": "未设定",
  "settings35": "未设定",
  "settings4": "🏷️ 兴趣标签：{user.interests }\n",
  "settings5": "📖 个人简介：{updatedUser.bio}\n",
  "settings6": "🌍 地区：{updatedUser.city}\n",
  "settings7": "📖 个人简介：{user.bio}\n",
  "settings8": "🌍 地区：{user.city }\n",
  "settings9": "你可以随时使用 /mbti 指令重新设定。",
  "short": "💡 您可以:",
  "short10": "工作时，你更喜欢：",
  "short100": "情感与故事",
  "short101": "效率和结果",
  "short102": "共识和团结",
  "short103": "自由和弹性",
  "short104": "保留选择权",
  "short105": "添加LINE",
  "short106": "测验结果",
  "short107": "先听后说",
  "short108": "小而亲密",
  "short109": "团队合作",
  "short11": "阅读时，你更喜欢：",
  "short110": "独立工作",
  "short111": "边说边想",
  "short112": "独自消化",
  "short113": "实际应用",
  "short114": "创新想法",
  "short115": "新的尝试",
  "short116": "合理吗？",
  "short117": "有益吗？",
  "short118": "坚持原则",
  "short119": "维持关系",
  "short12": "工作中，你更重视：",
  "short120": "公正果断",
  "short121": "体贴关怀",
  "short122": "整齐有序",
  "short123": "随性自在",
  "short124": "快速决定",
  "short125": "感到不安",
  "short126": "感到兴奋",
  "short127": "访问链接",
  "short128": "订阅频道",
  "short129": "银行帐号",
  "short13": "规划未来时，你会：",
  "short130": "备注：",
  "short131": "发送中",
  "short132": "信用卡",
  "short133": "比特币",
  "short134": "添加微信",
  "short135": "添加QQ",
  "short136": "手机号",
  "short137": "联系我",
  "short138": "一夜情",
  "short139": "性服务",
  "short14": "分析问题并提供建议",
  "short140": "骗钱",
  "short141": "投资",
  "short142": "赚钱",
  "short143": "汇款",
  "short144": "转帐",
  "short145": "密码",
  "short146": "传销",
  "short147": "金融",
  "short148": "理财",
  "short149": "股票",
  "short15": "面对变化，你通常：",
  "short150": "期货",
  "short151": "外汇",
  "short152": "电话",
  "short153": "约炮",
  "short154": "援交",
  "short155": "自杀",
  "short156": "跳楼",
  "short157": "暴力",
  "short158": "未设置",
  "short159": "未生成",
  "short16": "感谢你的支持！ ❤️",
  "short160": "测试用户",
  "short161": "测试用户",
  "short162": "结果更准确",
  "short163": "需要关注",
  "short164": "加入群组",
  "short165": "查看详情",
  "short17": "让其他用户更了解你",
  "short18": "这正确吗？\n\n",
  "short19": "🗑️ 删除草稿",
  "short2": "🌈 任何人都可以",
  "short20": "🏷️ 编辑兴趣",
  "short21": "请选择测验版本：",
  "short22": "您的工作风格是:",
  "short23": "有明确的截止日期",
  "short24": "很快就能熟络起来",
  "short25": "需要时间慢慢熟悉",
  "short26": "压力大时，你会：",
  "short27": "列清单按计划购买",
  "short28": "卡住（重试中）",
  "short29": "✏️ 修改内容",
  "short3": "✏️ 继续编辑资料",
  "short30": "📖 编辑简介",
  "short31": "🌍 编辑地区",
  "short32": "💝 匹配偏好",
  "short33": "正在更新...",
  "short34": "🔞 色情内容",
  "short35": "主动与他人交谈",
  "short36": "等待他人来找我",
  "short37": "周末你更喜欢：",
  "short38": "实际经验和事实",
  "short39": "一步一步的方法",
  "short4": "📝 重新进行测验",
  "short40": "探索创新的方式",
  "short41": "逻辑和客观分析",
  "short42": "情感和人际和谐",
  "short43": "提前计划和准备",
  "short44": "随机应变和灵活",
  "short45": "保持开放的选择",
  "short46": "使用比喻和类比",
  "short47": "倾听并给予安慰",
  "short48": "你的房间通常：",
  "short49": "购物时，你会：",
  "short5": "📝 进行快速测验",
  "short50": "让别人更了解你",
  "short51": "在同一城市找朋友",
  "short52": "看看别人的故事",
  "short53": "至少20个字符",
  "short54": ": 主动配对,",
  "short55": "女生（默认）",
  "short56": "男生（默认）",
  "short57": "结果仅供参考",
  "short58": "您可以:",
  "short59": "和朋友一起出去玩",
  "short6": "至少需要一个过滤器",
  "short60": "在家独处休息",
  "short61": "感到充满活力",
  "short62": "感到需要休息",
  "short63": "直觉和可能性",
  "short64": "关注具体细节",
  "short65": "关注整体概念",
  "short66": "直接指出问题",
  "short67": "考虑对方感受",
  "short68": "制定详细行程",
  "short69": "随心所欲探索",
  "short7": "新用户******",
  "short70": "积极发表意见",
  "short71": "您的朋友圈:",
  "short72": "广泛但不深入",
  "short73": "未来和可能性",
  "short74": "使用具体例子",
  "short75": "已验证的方法",
  "short76": "基于现实条件",
  "short77": "想像各种可能",
  "short78": "有规律和结构",
  "short79": "包含敏感词汇",
  "short8": "批评他人时，你会：",
  "short80": "填写兴趣标签",
  "short81": "完善自我介绍",
  "short82": "加入官方频道",
  "short83": "写下你的故事",
  "short84": "👨 男性",
  "short85": "👩 女性",
  "short86": "👨 男",
  "short87": "👩 女",
  "short88": "审核时间：",
  "short89": "❓ 不确定",
  "short9": "旅行时，你倾向于：",
  "short90": "您更看重什么:",
  "short91": "公平和正义",
  "short92": "同情与理解",
  "short93": "先想好再说",
  "short94": "与朋友聊天",
  "short95": "实用的指南",
  "short96": "理论和概念",
  "short97": "现在和过去",
  "short98": "你更信任谁:",
  "short99": "事实和数据",
  "start": "开始时间：{new Date(maintenance.startTime).toLocaleString('zh-TW')}\n",
  "start10": "开始使用 →",
  "start2": "开始时间: {new Date(broadcast.startedAt).toLocaleString('zh-TW')}\n",
  "start3": "开始：{startTime.toLocaleString('zh-TW')}\n",
  "start4": "开始时间: {broadcast.started_at}\n\n",
  "start5": "开始：{startTime.toLocaleString(",
  "start6": "🎉 **准备好了！开始交朋友吧～**\n\n",
  "start7": "• 使用 /throw 重新开始\n",
  "start8": "📺 开始观看广告",
  "start9": "开始你的交友之旅",
  "stats": "💡 使用 /ad_stats {id} 查看详细统计",
  "stats2": "• /stats - 查看统计\n\n",
  "stats3": "📊 查看统计 - /stats\n",
  "stats4": "📊 **官方广告统计**\n\n",
  "stats5": "邀请统计:\n",
  "stats6": "统计:\n",
  "success": "购买成功",
  "systemError": "❌ 系统发生错误",
  "task": "🎉 恭喜完成任务：加入官方频道！\n\n",
  "task2": "[📋 查看任务中心] → /tasks",
  "task3": "• /tasks - 查看任务中心\n",
  "task4": "💡 使用 /tasks 查看更多任务",
  "task5": "处理任务时，你会：",
  "task6": "📋 查看任务",
  "text": "目标：{broadcast.target_type}\n",
  "text10": "📖 个人简介：{updatedUser.bio ||",
  "text100": "💡 请在下方输入框输入内容",
  "text101": "这可能需要一些时间，请稍候。",
  "text102": "你可以随时使用以下命令：\n",
  "text103": "🛠️ 系统维护通知\n\n",
  "text104": "🛠️ 维护模式状态\n\n",
  "text105": "评价一个想法时，你首先考虑：",
  "text106": "⏱️ 约 2-3 分钟\n",
  "text107": "⏱️ 约 5-8 分钟\n",
  "text108": "📚 我想了解更多安全知识",
  "text109": "立即发送（约 1-2 秒）",
  "text11": "{daysAgo} 天前",
  "text110": "2️⃣ 选择「回覆」\n",
  "text111": "**过滤器格式：**\n",
  "text112": "• 最短 5 个字符\n",
  "text113": "• 不能包含网址连结\n",
  "text114": "请输入你的地区：\n\n",
  "text115": "• 你可以随时修改此设置",
  "text116": "• 最多 5 个标签\n",
  "text117": "📋 快速版（12 题）",
  "text118": "📚 完整版（36 题）",
  "text119": "• 进行更详细的测验\n",
  "text12": "总经理 - 出色的管理者，在管理事务或人员方面无与伦比。",
  "text120": "💡 **提示:**",
  "text121": "学习新事物时，你更喜欢：",
  "text122": "**操作步骤：**\n",
  "text123": "🇺🇳 使用联合国旗",
  "text124": "要直接发送这个草稿吗？",
  "text125": "这可能需要几秒钟时间。",
  "text126": "感谢您的耐心等待！",
  "text127": "维护时长最少 5 分钟",
  "text128": "在社交场合中，你通常：",
  "text129": "解决问题时，你更依赖：",
  "text13": "💡 这将显示在你的资料卡上，让其他用户更了解你。\n",
  "text130": "在团队中，你更倾向于：",
  "text131": "思考问题时，你倾向于：",
  "text132": "描述事物时，你倾向于：",
  "text133": "朋友向你倾诉时，你会：",
  "text134": "团队决策时，你更关注：",
  "text135": "你认为好的领导者应该：",
  "text136": "目标: 所有用户\n",
  "text137": "✏️ 请输入新的内容",
  "text138": "💰 诈骗 / 钓鱼",
  "text139": "😡 骚扰 / 辱骂",
  "text14": "XunNi 致力于提供安全的交友环境，请注意以下事项：",
  "text140": "参加聚会后，你通常：",
  "text141": "做决定时，你更重视：",
  "text142": "遇到新朋友时，你会：",
  "text143": "冲突中，你更倾向于：",
  "text144": "你更容易被说服通过：",
  "text145": "你更喜欢的生活方式：",
  "text146": "做决定时，你倾向于：",
  "text147": "随意逛逛看到喜欢就买",
  "text148": ")} 到期。\n\n",
  "text149": "📋 法律文档仅提供英文版本。",
  "text15": "2. 涉及金钱交易请务必谨慎",
  "text150": "📋 法的文书は英语版のみ提供されています。",
  "text16": "5. 发现违规用户请使用 /report",
  "text17": "若您遇到可疑情况，请立即封锁并举报该用户。",
  "text18": "⚠️ **风险提示**",
  "text19": "/broadcast_cleanup 确认",
  "text2": "请使用 /broadcast_filter 查看正确格式。",
  "text20": "💝 匹配偏好: {matchPrefText}",
  "text21": "辩论家 - 聪明好奇的思想家，无法抗拒智力上的挑战。",
  "text22": "写下你的心情或想法，系统会帮你找到合适的人\n\n",
  "text23": "物流师 - 实际且注重事实的个人，可靠性不容怀疑。",
  "text24": "鉴赏家 - 大胆而实际的实验者，擅长使用各种工具。",
  "text25": "💡 这是一次性奖励，领取后会追加到今天的额度中。",
  "text26": "符合用户数: {totalUsers} 人\n",
  "text27": "• 默认为异性（男生寻找女生，女生寻找男生）\n",
  "text28": "状态: {progress.status}\n",
  "text29": "{daysAgo} 小时前",
  "text3": "**过滤条件：**\n{filtersDesc}\n\n",
  "text30": "约 {remainingMinutes} 分钟",
  "text31": "约 {hours} 小时 {mins} 分钟",
  "text32": "维护时长不能超过 24 小时（1440 分钟）",
  "text33": "用户数: {totalUsers} 人\n",
  "text34": "目标: {b.target_type}\n",
  "text35": "过滤条件: {filtersDesc}\n",
  "text36": "• 最少 4 个字符，最多 36 个字符\n",
  "text37": "🇺🇳 如果找不到，可以选择「联合国旗」",
  "text38": "📖 个人简介：{user.bio ||",
  "text39": "请输入你的兴趣标签（用逗号分隔）：\n\n",
  "text4": "/maintenance_enable 60 系统升级维护",
  "text40": "• 例如：音乐, 电影, 旅行, 美食\n",
  "text41": "服务已恢复正常，感谢您的耐心等待！\n\n",
  "text42": "🌍 地区：{user.city ||",
  "text43": "来源：{sourceText}\n\n",
  "text44": "未知的过滤器：{trimmedKey}",
  "text45": "系统正在进行维护，暂时无法使用。\n\n",
  "text46": "我们根据你的语言设置，推测你来自：\n",
  "text47": "• 每个标签最多 20 个字符\n\n",
  "text48": "时长：{duration} 分钟\n",
  "text49": "1. 你了解网路交友的安全风险吗？\n",
  "text5": "👋 欢迎回来，{user.nickname}！\n\n",
  "text50": "2. 你会保护好自己的个人资讯吗？\n",
  "text51": "很好！现在请上传你的头像照片：\n\n",
  "text52": "🌊 **XunNi 是什么？ **\n",
  "text53": "🎉 检测到你已加入官方频道！\n\n",
  "text54": "💡 这样可以准确指定要封锁的对象。",
  "text55": "状态：{statusText}\n",
  "text56": "💡 现在可以直接测试核心功能：\n",
  "text57": "你想要寻找什么样的聊天对象？\n\n",
  "text58": "• 介绍你的兴趣、性格或想说的话\n",
  "text59": "🏷️ **编辑兴趣标签**\n\n",
  "text6": "建筑师 - 富有想像力和战略性的思想家，一切皆在计划之中。",
  "text60": "为了安全，只允许以下网域的连结：\n",
  "text61": "📋 **快速版（12 题）**\n",
  "text62": "📚 **完整版（36 题）**\n",
  "text63": "• 头像会自动每 7 天更新一次\n",
  "text64": "3️⃣ 输入 /report\n\n",
  "text65": "3️⃣ 输入 /block\n\n",
  "text66": "状态: {b.status}\n",
  "text67": "不会再被自动处理或重新发送\n\n",
  "text68": "📖 **编辑个人简介**\n\n",
  "text69": "💝 **设置匹配偏好**\n\n",
  "text7": "主人公 - 富有魅力且鼓舞人心的领导者，有能力使听众着迷。",
  "text70": "💬 **你的聊天记录**\n\n",
  "text71": "• 您也可以随时使用此命令手动刷新",
  "text72": "📊 **每日数据分析报表**\n",
  "text73": "你的帐号已恢复为免费会员。\n\n",
  "text74": "💡 这将显示在你的资料卡上\n",
  "text75": "🔧 开发模式：用户信息\n\n",
  "text76": "• 直接输入新内容来替换草稿\n",
  "text77": "• 不允许连结、图片、多媒体\n",
  "text78": "• 显示时最多 18 个字符\n",
  "text79": "• 对方最多显示 18 个字\n",
  "text8": "/broadcast 系统将于今晚 22:00 进行维护",
  "text80": "💡 请移除这些连结后重新发送。",
  "text81": "🔄 正在刷新头像...\n\n",
  "text82": "• 免费用户看到的是模糊头像\n",
  "text83": "💬 **如何成为朋友？ **\n",
  "text84": "无效的过滤器格式：{pair}",
  "text85": "你的所有数据已被删除。\n\n",
  "text86": "📝 **草稿内容**\n\n",
  "text87": "🌍 **编辑地区**\n\n",
  "text88": "• 例如：台北、香港、东京\n",
  "text89": "• 最多 50 个字符\n\n",
  "text9": "🏷️ 兴趣标签：{user.interests ||",
  "text90": "约 {minutes} 分钟",
  "text91": "写下你的故事（至少 20 字）",
  "text92": "正在查询符合条件的用户...",
  "text93": "• 最多 250 个字符\n",
  "text94": "• 不要包含个人联络方式\n",
  "text95": "请选择要编辑的项目：\n\n",
  "text96": "请输入你的个人简介：\n\n",
  "text97": "• 最多 200 个字符\n",
  "text98": "• 避免包含联络方式\n\n",
  "text99": "现在可以正常使用所有功能了。",
  "throw": "⏰ 丢瓶流程已超时\n\n请使用 /throw 重新开始。",
  "throw2": "丢瓶流程",
  "uncertain": "❓ 不确定",
  "unknownOption": "⚠️ 未知的选项",
  "unlimited": "无限制",
  "userNotFound": "❌ 用户不存在",
  "vip": "你的 VIP 订阅已于 {expireDate} 到期。\n\n",
  "vip10": "😢 **VIP 订阅已到期**\n\n",
  "vip11": "• 升级 VIP 后会自动刷新历史帖子",
  "vip12": "⭐ 升级 VIP - /vip\n",
  "vip13": "💎 VIP 用户无需观看广告",
  "vip14": "目标: 非 VIP 用户\n",
  "vip15": "目标: VIP 用户\n",
  "vip16": "非 VIP 用户",
  "vip17": "VIP 用户",
  "vip2": "你的 VIP 订阅已于 {new Date(user.vip_expire_at).toLocaleDateString(",
  "vip3": "/broadcast_filter vip=true,mbti=INTJ VIP 专属活动通知\n",
  "vip4": "每邀请 1 人，每日额度永久 +1（免费最多 10 人，VIP 最多 100 人）",
  "vip5": "VIP: {user.is_vip ? '是' : '否'}",
  "vip6": "💡 升级 VIP 可使用进阶筛选（MBTI/星座）：/vip",
  "vip7": "💡 随时可以重新订阅 VIP：/vip\n\n",
  "vip8": "💡 血型可用于 VIP 血型配对功能\n\n",
  "vip9": "• VIP 用户可以看到清晰的对方头像\n",
  "yes": "是",
  "zodiac": "无效的星座：{trimmedValue}（必须是以下之一：{VALID_ZODIACS.join(', ')}）",
  "saved_to_history": "已保存到历史记录",
  "analyzing": "分析中...",
  "date": "日期：{date}",
  "processing": "⏳ 处理中...",
  "continue_op": "接下来...",
  "unknown": "未知",
  "no_reselect": "否，手动选择",
  "fortuneBottle": "算命瓶",
  "weeklyFree": "本周免费",
  "additional": "额外",
  "confirm_action": "确认操作",
  "continue": "继续",
  "done": "完成",
  "edit": "编辑",
  "ok": "好",
  "save": "保存",
  "skip": "跳过",
  "submit": "提交",
  "timeLeftDaysHours": "{days} 天 {hours} 小时",
  "timeLeftHours": "{hours} 小时",
  "confirm_delete": "确认删除",
  "deleted": "已删除",
  "unblock": "解除封锁",
  "generating": "正在生成...",
  "mbtiMatch": "MBTI 契合度",
  "zodiacMatch": "星座契合度",
  "fateMatch": "缘分指数"
};
