export default {
  ...
  "grant": {
    "successVip": "✅ 已成功为用户 {userId} ({nickname}) 开通 VIP 权限 (30天)。",
    "successBottles": "✅ 已成功为用户 {userId} ({nickname}) 增加 50 个算命瓶。",
    "invalidId": "❌ 无效的用户 ID。",
    "userNotFound": "❌ 找不到用户 {userId}。",
    "onlySuperAdmin": "❌ 此指令仅限超级管理员使用。",
    "usageVip": "用法: /add_vip [用户ID] (不填则为自己)",
    "usageBottles": "用法: /add_bottles [用户ID] (不填则为自己)"
  }
};
