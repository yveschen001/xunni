...
`,
    adConfig: {
      ...
      disableCommand: `• \`/ad_provider_disable \` - تعطيل \`/ad_provider_disable <id>\``,
      disableFailed: `❌ فشل تعطيل مقدم الإعلانات`,
      disableOfficialAdCommand: `• \`/official_ad_disable \` - تعطيل \`/official_ad_disable <id>\``,
      disableOfficialAdFailed: `❌ فشل تعطيل الإعلان الرسمي`,
      disabled: `❌ تعطيل`,
      enableCommand: `• \`/ad_provider_enable \` - تفعيل \`/ad_provider_enable <id>\``,
      enableFailed: `❌ فشل تفعيل مقدم الإعلانات`,
      enableOfficialAdCommand: `• \`/official_ad_enable \` - تفعيل \`/official_ad_enable <id>\``,
      ...
      priorityCommand: `• \`/ad_provider_priority \` - تعيين الأولوية \`/ad_provider_priority <id> <priority>\``,
      ...
      viewStatsCommand: `• \`/ad_stats \` - عرض إحصائيات مفصلة \`/ad_stats <id>\``,
      weight: `• الوزن: \${weight}`,
    },
    analytics: {
...
`,
      ...
      catch: `📊 اختيارات اليوم: \${newCatchesCount}/\${quota}
...
`,
      settings: `🧠 MBTI: \\\${mbti}
 \${mbti}`,
      settings2: `⭐ الأبراج: \\\${zodiac}
 \${zodiac}`,
      settings3: `🧠 MBTI: \${mbti}
 \${mbti}`,
      settings4: `⭐ برج: \${zodiac}
 \${zodiac}`,
...
`,
      zodiac: `• برج: \\\\$\\{selectedZodiac.length > 0 ? selectedZodiac.map((z) => ZODIAC_NAMES[z]).join(', ') : 'لا حدود'}
`,
...
`,
    ...
    filter: {
      ...
      invalidCountry: `رمز البلد غير صالح: {value} (يجب أن يكون حرفين كبيرين، مثل: TW، US، JP)`,
...
• معدل النقر (CTR): \${stats.ctr}% {ad.is_enabled ? '啟用' : '停用'} \${status}`,
    message10: `/broadcast_filter gender=female,age=18-25,country=TW مرحبًا للجميع!
`,
    message11: `\${banHours} \${user.language_pref === 'en' ? 'ساعات' : '小時'} {user.language_pref === 'en' ? 'hours' : '小時'} \${user.language_pref === 'en' ? 'hours' : '小時'}`,
...
`,
    message15: `\${days} \${user.language_pref === 'en' ? 'أيام' : '天'} {user.language_pref === 'en' ? 'days' : '天'} \${user.language_pref === 'en' ? 'days' : '天'}`,
...
`,
    ...
    selected: `محدد`,
    settings: `🧠 MBTI: \${updatedUser.mbti_result} (يمكن إعادة الاختبار) {updatedUser.mbti_result || '未設定'} \${updatedUser.mbti_result}`,
...
`,
 ...
 {updatedUser.interests || '未設定'} \${updatedUser.interests}`,
    ...
    settings3: `🧠 MBTI: \${mbti} (إعادة اختبار متاحة) \${mbti}`,
...
如有疑問，請使用 /appeal 申訴。`,
      ...
      broadcast: `❌ نظام البث الحالي يدعم فقط البث لغاية \\\\$MAX_SAFE_USERS مستخدمين.
...
يرجى إعادة الإدخال أو إلغاء التحرير:`,
      ...
      cancel7: `❌ تم الإلغاء \\\\$ZODIAC_NAMES[zodiacSign] {ZODIAC_NAMES[zodiacSign]} \${ZODIAC_NAMES[zodiacSign]}`,
...
رسالة الخطأ: \\\\$\\{error instanceof Error ? error.message : String(error)} \${error instanceof Error ? error.message : String(error)}`,
      message4: `❌ نعتذر، يجب أن تكون على الأقل 18 عامًا لاستخدام هذه الخدمة.
...
`,
    useTelegramNickname: `استخدام اسم Telegram`,
...
`,
 ...
 {user.language_pref || 'zh-TW'} \${user.language_pref}`,
...
`,
    ...
    vip: `عضو VIP (تاريخ انتهاء الصلاحية: \${expireDate}） {new Date(user.vip_expire_at).toLocaleDateString('zh-TW')} \${expireDate}`,
...
يرجى اختيار لغتك المفضلة:`,
    returnToMenu: `[Translation Needed: settings.returnToMenu]`,
    selectOption: `[Translation Needed: settings.selectOption]`,
...
`,
    mbti: `🧠 **MBTI**：\${mbti}
`,
    message: `• تاريخ انتهاء الصلاحية：\${expireDate}
`,
...
`,
 ...
 {gender === 'male' ? '👨 男性' : '👩 女性'} \${gender}`,
      nickname: `✅ باستخدام اسم مستخدم تلغرام: \\\\$\${suggestedNickname.substring(0, 18)}`,
      nickname2: `✅ تم تحديث الاسم المستعار إلى: \\\\$\${text}
...
`,
 ...
 {new Date(req.requested_at).toLocaleString('zh-TW')} \${new Date(req.requested_at).toLocaleString('zh-TW')}`,
    message3: `وقت الانتهاء الجديد: \${expireDate}

 {newExpire.toLocaleDateString('zh-TW')} \${expireDate}`,
    message4: `وقت الانتهاء: \${expireDate}
