...
export class UserPreferencesService {
  ...
  async getPreferences(userId: string): Promise<UserPushPreferences> {
    const prefsResult = await this.db
      .prepare('SELECT * FROM user_push_preferences WHERE user_id = ?')
      .bind(userId)
      .first<any>();
  ...
  async updatePreferences(userId: string, prefs: Partial<UserPushPreferences>): Promise<void> {
    ...
    if (updates.length > 0) {
      ...
      const exists = await this.db
        .prepare('SELECT 1 FROM user_push_preferences WHERE user_id = ?')
        .bind(userId)
        .first();
      ...
      if (exists) {
        ...
        `
          ...
          .bind(
            userId,
            newPrefs.quiet_hours_start,
            newPrefs.quiet_hours_end,
            newPrefs.throw_reminder_enabled,
            newPrefs.catch_reminder_enabled,
            newPrefs.message_reminder_enabled,
            newPrefs.onboarding_reminder_enabled,
