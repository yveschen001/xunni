      ...
      try {
        ...
        await telegram.sendMessageWithButtons(
          ...
          [
            ...
            [{ text: '❌ 稍後再說', callback_data: 'vip_cancel_reminder' }],
          ]
        );
...
export async function createOrUpdateSubscription(
  ...
  try {
    ...
    const existing = await db.d1.prepare(`
    ...
    if (existing) {
      ...
      await db.d1.prepare(`
        ...
        SET expire_date = ?,
            ...
            updated_at = datetime('now')
        WHERE id = ?
      `).bind(expireDate.toISOString(), paymentId, existing.id).run();
    } else {
      ...
      `).bind(userId, expireDate.toISOString(), paymentId).run();
    }
  } catch (error) {
