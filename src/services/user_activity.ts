...
export async function getUserActivityLevel(
  ...
  try {
    ...
    console.error(`[getUserActivityLevel] Error for ${telegramId}:`, e);
    return UserActivityLevel.ACTIVE; // Fallback to active to avoid missing push if error
  }
}
...
export async function updateUserActivity(
  ...
  try {
    ...
    console.log(
      `[updateUserActivity] Updated ${result.meta?.changes || 0} rows for user ${telegramId}`
    );
  } catch (error) {
