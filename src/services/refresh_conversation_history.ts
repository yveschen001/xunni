  ...
  try {
    ...
    for (const post of historyPosts.results) {
      try {
        ...
        const partnerId = post.user_a_telegram_id === userId 
          ? post.user_b_telegram_id 
          : post.user_a_telegram_id;
        ...
        console.error('[RefreshHistory] Got avatar file_id:', partnerAvatarFileId?.substring(0, 50));
        ...
        console.error('[RefreshHistory] Preparing to send message. Avatar file_id:', partnerAvatarFileId?.substring(0, 50));
        
        if (partnerAvatarFileId) {
          // Send with photo using file_id
          try {
            console.error('[RefreshHistory] Sending photo message with file_id...');
            const sentMessage = await telegram.sendPhoto(
              parseInt(userId),
              partnerAvatarFileId,
              {
                caption: newContent,
            ...
            console.error('[RefreshHistory] Failed to send photo, falling back to text:', photoError);
            // Fallback to text
...
export async function refreshConversationHistoryInBackground(
  ...
  refreshAllConversationHistoryPosts(db, env, userId)
    .then(result => {
      console.log(`[RefreshHistoryBg] Successfully refreshed ${result.updated} posts for user ${userId}`);
    })
    .catch(error => {
