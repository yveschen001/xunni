  ...
  try {
    ...
    for (const user of expiredUsers.results as any[]) {
      try {
        ...
        const userForI18n = await findUserByTelegramId(db, user.telegram_id);
        ...
        console.error(
          `[checkExpiredSubscriptions] Successfully processed user ${user.telegram_id}`
        );
      } catch (error) {
        console.error(
          `[checkExpiredSubscriptions] Failed to process user ${user.telegram_id}:`,
          error
        );
      }
    }
