...
output_file = 'i18n_for_translation_fixed_v4.csv'
...
def fix_text(text, key):
    ...
    if "gender" in text and "?" in text:
        # Handle user.gender, updatedUser.gender, otherUser.gender
        text = re.sub(r'\$\{[a-zA-Z0-9_]*\.?user\.gender[^\}]+\}', '${gender}', text)
        text = re.sub(r'\$\{[a-zA-Z0-9_]*\.gender[^\}]+\}', '${gender}', text) # Fallback for simple prop
    ...
    if text != original_text:
        print(f"Fixed {key}: {original_text[:50]}... -> {text[:50]}...")
    return text
