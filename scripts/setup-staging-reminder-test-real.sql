...
-- Insert a dummy bottle for the conversation (if not exists)
INSERT INTO bottles (id, owner_telegram_id, content, status)
VALUES (99999, '999999999', 'Dummy Bottle Content', 'matched')
ON CONFLICT(id) DO NOTHING;

-- Insert active conversation for YOU (396943893)
-- Partner spoke last, 25 hours ago
...
VALUES ('396943893', '999999999', 99999, 'active', datetime('now', '-30 hours'), datetime('now', '-25 hours'), '999999999');

-- Insert message from Partner
...
SELECT id, '999999999', '396943893', '這是一條給 396943893 的測試訊息！請在 24 小時內回覆我！', datetime('now', '-25 hours'), 0
FROM conversations 
WHERE user_a_telegram_id = '396943893' AND user_b_telegram_id = '999999999' AND status = 'active'
